...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  nh.param("exploration/zombie_frontier_repeat", ep_->zombie_frontier_repeat_, 0);
  nh.param("exploration/viewpoint_visibility_recheck", ep_->viewpoint_visibility_recheck_, false);
  nh.param("exploration/unreachable_frontier_repeat", ep_->unreachable_frontier_repeat_, 0);
  nh.param("exploration/frontier_exit_fixes", ep_->frontier_exit_fixes_, false);
  nh.param("exploration/zombie_frontier_block_ttl", ep_->zombie_frontier_block_ttl_, 90.0);
  nh.param("exploration/zombie_frontier_block_radius", ep_->zombie_frontier_block_radius_, 3.0);
...
int FastExplorationManager::planExploreMotion(
  ...
  ed_->last_plan_requires_grid_release_ = false;
  ed_->last_fallback_grid_ = -1;
  ed_->last_plan_no_progress_ = false;
  ...
  // Frontiers retired by the zombie logic stay in frontiers_ until the next
  // searchFrontiers(); skip them here so a static PLAN_TRAJ retry loop cannot
  // re-select one before the refresh.
  if (!frontier_ids.empty() && frontier_finder_ != nullptr) {
    const size_t before = frontier_ids.size();
    frontier_ids.erase(
        std::remove_if(frontier_ids.begin(), frontier_ids.end(),
            [&](int id) {
              return id >= 0 && id < static_cast<int>(ed_->averages_.size()) &&
                     frontier_finder_->isFrontierBlocked(ed_->averages_[id]);
            }),
        frontier_ids.end());
    if (frontier_ids.size() != before)
      ROS_WARN("RACER_METRIC blocked_frontiers_skipped drone=%d count=%zu", ep_->drone_id_,
          before - frontier_ids.size());
  }
  ...
  if (ep_->min_translation_progress_ > 0.0 &&
    ...
    ed_->last_plan_requires_grid_release_ = true;
    // Record the rejected goal so the FSM's plan-fail watchdog blocks THIS
    // point (it used to block the previous, unrelated next_pos_).  next_pos_
    // itself is left untouched: the FSM may still be executing towards it.
    ed_->last_plan_no_progress_ = true;
    ed_->last_rejected_pos_ = next_pos;
    ed_->last_rejected_yaw_ = next_yaw;
    const double total_wall_ms =
        (ros::WallTime::now() - wall_plan_start).toSec() * 1000.0;
    ROS_ERROR(
        ...
        target_translation, ep_->min_translation_progress_);
    // Zombie-frontier exit.  The robot is at the viewpoint and the frontier
    // has survived the latest scans: seeing it again cannot change anything
    // (unknown behind it is occluded / inside a slab / in the LiDAR's blind
    // cone).  Block the viewpoint and retire the frontier so the tour moves
    // on instead of re-selecting the same point every replan cycle.
    if (ep_->zombie_frontier_repeat_ > 0) {
      const bool same_target = (next_pos - no_progress_target_).norm() < 0.5;
      if (!same_target)
        no_progress_repeat_ = 1;
      else if (ed_->frontier_update_seq_ != no_progress_seq_)
        ++no_progress_repeat_;  // count once per frontier refresh only
      no_progress_seq_ = ed_->frontier_update_seq_;
      no_progress_target_ = next_pos;
      if (no_progress_repeat_ >= ep_->zombie_frontier_repeat_) {
        const double until = ros::Time::now().toSec() + ep_->zombie_frontier_block_ttl_;
        frontier_finder_->blockViewpoint(next_pos, until);
        Eigen::Vector3d average = next_pos;
        if (!frontier_ids.empty() && frontier_ids.front() >= 0 &&
            frontier_ids.front() < static_cast<int>(ed_->averages_.size()))
          average = ed_->averages_[frontier_ids.front()];
        frontier_finder_->blockFrontier(average, ep_->zombie_frontier_block_radius_, until);
        ROS_ERROR(
            "RACER_RECOVERY drone=%d zombie_frontier_blocked=1 grid=%d "
            "frontier=%d average=[%.2f,%.2f,%.2f] repeats=%d ttl=%.0f",
            ep_->drone_id_, grid_ids.front(), frontier_ids.empty() ? -1 : frontier_ids.front(),
            average[0], average[1], average[2], no_progress_repeat_,
            ep_->zombie_frontier_block_ttl_);
        no_progress_repeat_ = 0;
      }
    }
    ROS_WARN(
  ...
  no_progress_repeat_ = 0;  // a target with real translation was accepted

  // Pre-flight check: is the chosen frontier still visible from the chosen
  // viewpoint on the CURRENT map?  Viewpoints are sampled once when a
  // frontier is created; walls mapped since then (own scans or a peer's
  // chunk with 0.3-0.7 m frame error) can make every stored viewpoint
  // blind, and the frontier then survives arrival forever.  Block such a
  // target now instead of flying there and looping.
  if (ep_->viewpoint_visibility_recheck_ && !frontier_ids.empty()) {
    const int visib =
        frontier_finder_->recheckViewpointVisibility(frontier_ids.front(), next_pos, next_yaw);
    if (visib >= 0 && visib <= frontier_finder_->minVisibNum()) {
      const double until = ros::Time::now().toSec() + ep_->zombie_frontier_block_ttl_;
      frontier_finder_->blockViewpoint(next_pos, until);
      Eigen::Vector3d average = next_pos;
      if (frontier_ids.front() >= 0 &&
          frontier_ids.front() < static_cast<int>(ed_->averages_.size()))
        average = ed_->averages_[frontier_ids.front()];
      frontier_finder_->blockFrontier(average, ep_->zombie_frontier_block_radius_, until);
      ed_->last_plan_no_progress_ = true;
      ...
      const double total_wall_ms =
          (ros::WallTime::now() - wall_plan_start).toSec() * 1000.0;
      ROS_ERROR(
          "RACER_RECOVERY drone=%d viewpoint_invisible=1 grid=%d frontier=%d visible=%d "
          "min=%d target=[%.2f,%.2f,%.2f]",
          ep_->drone_id_, grid_ids.front(), frontier_ids.front(), visib,
          frontier_finder_->minVisibNum(), next_pos[0], next_pos[1], next_pos[2]);
      ROS_WARN(
          "RACER_METRIC full_plan drone=%d result=%d grids=%zu frontiers=%zu "
          "route_ms=%.3f viewpoint_ms=%.3f trajectory_ms=0.000 total_ms=%.3f",
          ep_->drone_id_, FAIL, grid_ids.size(), frontier_ids.size(), route_wall_ms,
          viewpoint_wall_ms, total_wall_ms);
      return FAIL;
    }
  }
  ...
  if (planTrajToView(pos, vel, acc, yaw, next_pos, next_yaw) == FAIL) {
    // Unreachable-frontier exit: A* from a clear start keeps failing towards
    // the same frontier (its viewpoints sit in a free pocket that inflation or
    // unknown space seals off from here).  Retire it instead of cycling
    // through its viewpoints four failures at a time.
    if (ep_->unreachable_frontier_repeat_ > 0 && ed_->last_plan_failed_astar_ &&
        planner_manager_->path_finder_->lastStartOccupancy() != SDFMap::OCCUPIED &&
        !frontier_ids.empty() &&
        frontier_ids.front() >= 0 &&
        frontier_ids.front() < static_cast<int>(ed_->averages_.size())) {
      const Eigen::Vector3d average = ed_->averages_[frontier_ids.front()];
      if ((average - astar_fail_average_).norm() < 1.0)
        ++astar_fail_repeat_;
      else
        astar_fail_repeat_ = 1;
      astar_fail_average_ = average;
      if (astar_fail_repeat_ >= ep_->unreachable_frontier_repeat_) {
        const double until = ros::Time::now().toSec() + ep_->zombie_frontier_block_ttl_;
        frontier_finder_->blockViewpoint(next_pos, until);
        frontier_finder_->blockFrontier(average, ep_->zombie_frontier_block_radius_, until);
        ROS_ERROR(
            "RACER_RECOVERY drone=%d unreachable_frontier_blocked=1 grid=%d frontier=%d "
            "average=[%.2f,%.2f,%.2f] failures=%d ttl=%.0f",
            ep_->drone_id_, grid_ids.front(), frontier_ids.front(), average[0], average[1],
            average[2], astar_fail_repeat_, ep_->zombie_frontier_block_ttl_);
        astar_fail_repeat_ = 0;
      }
    }
  ...
  astar_fail_repeat_ = 0;  // trajectory planned: the frontier was reachable
  const double trajectory_wall_ms =
      (ros::WallTime::now() - wall_traj_start).toSec() * 1000.0;
...
int FastExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  ed_->last_plan_failed_astar_ = false;
  ed_->last_plan_requires_grid_release_ = false;
  // last_fallback_grid_ is owned by planExploreMotion (set in the frontier-less
  // branch just before this call).  Resetting it here hid every A*-failed
  // fallback target from the FSM's 3-failure grid release (v73 bot3: 1309
  // consecutive failures on one frontier-less cell).  Kept (gated) so the
  // planar quadruped launches reproduce their earlier behaviour.
  if (!ep_->frontier_exit_fixes_) ed_->last_fallback_grid_ = -1;
