...
void FastExplorationFSM::init(ros::NodeHandle& nh) {
  ...
  nh.param("fsm/frontier_exit_fixes", fp_->frontier_exit_fixes_, false);
      ...
      if (res == SUCCEED) {
        ...
        if (fp_->plan_fail_block_after_ > 0 &&
          ...
          const Eigen::Vector3d goal =
              (fp_->frontier_exit_fixes_ && expl_manager_->ed_->last_plan_no_progress_)
                  ? expl_manager_->ed_->last_rejected_pos_
    ...
    case EXEC_TRAJ: {
      ...
      if (!fd_->go_back_) {
        ...
        if (!need_replan && info->duration_ - t_cur < fp_->replan_thresh1_) {
          // Replan if traj is almost fully executed.  If the trajectory ends at
          // the committed target and the hold is not complete yet, simply keep
          // hovering there: a same-target replan from the trajectory end is a
          // zero-length path (no_progress_path FAIL loop) while the vehicle is
          // still converging on the end point.
          const Eigen::Vector3d traj_end =
              info->position_traj_.evaluateDeBoorT(info->duration_);
          const bool ends_at_target = fd_->target_initialized_ &&
                                      (traj_end - fd_->active_target_pos_).norm() < 0.5;
          if (fp_->frontier_exit_fixes_ && !hold_complete && ends_at_target) {
            ROS_WARN_THROTTLE(1.0,
                "RACER_METRIC target_hold_hover drone=%d elapsed_s=%.3f target_dist_m=%.3f",
                getId(), tn.toSec() - fd_->target_started_time_.toSec(),
                (fd_->odom_pos_ - fd_->active_target_pos_).norm());
          } else {
