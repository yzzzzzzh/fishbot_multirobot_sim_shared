...
public:
  ...
  // Consecutive no-progress rejections of one viewpoint (zombie-frontier exit).
  int no_progress_repeat_{0};
  uint64_t no_progress_seq_{0};
  // Consecutive A* failures towards one frontier (unreachable-frontier exit).
  int astar_fail_repeat_{0};
  Eigen::Vector3d astar_fail_average_{0.0, 0.0, 0.0};
  Eigen::Vector3d no_progress_target_{0.0, 0.0, 0.0};
