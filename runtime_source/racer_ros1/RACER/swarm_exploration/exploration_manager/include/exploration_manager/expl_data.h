...
struct ExplorationData {
  ...
  Vector3d next_pos_;
  // Goal rejected by the no-progress guard (zero-length target).  Kept apart
  // from next_pos_, which must keep meaning "goal of the trajectory the FSM
  // may still be executing" for the same-target replans.
  Vector3d last_rejected_pos_;
  double last_rejected_yaw_{0.0};
  bool last_plan_no_progress_{false};
  double next_yaw_;
...
struct ExplorationParam {
  ...
  // Zombie-frontier exit: after this many consecutive no-progress rejections
  // of the same viewpoint (the robot is already there but the frontier does
  // not clear) block the viewpoint and retire the frontier for the TTL.
  int zombie_frontier_repeat_;
  // Pre-flight re-check of the chosen viewpoint's frontier visibility on the
  // current map; an invisible target is blocked instead of flown to.
  bool viewpoint_visibility_recheck_;
  // Consecutive A* failures (clear start) towards one frontier before it is
  // retired as unreachable (0 = off).
  int unreachable_frontier_repeat_;
  // Master switch for the 3-D frontier-exit fixes (see single_drone_planner.xml).
  bool frontier_exit_fixes_;
  double zombie_frontier_block_ttl_, zombie_frontier_block_radius_;
