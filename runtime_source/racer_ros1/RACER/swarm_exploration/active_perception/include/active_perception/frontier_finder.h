...
struct Frontier {
  ...
  list<double> costs_;
  // >0: retired as a zombie (blockFrontier) until this ROS time; while
  // dormant its cells keep their frontier flag so the seed scan cannot
  // rebuild it every cycle.  After expiry the flags are cleared so the
  // cluster can re-form on the next map update of its region.
  double retire_until_{0.0};
};
...
public:
  ...
  void blockFrontier(const Eigen::Vector3d& average, double radius, double until);
  bool isFrontierBlocked(const Eigen::Vector3d& average) const;
  double frontierBlockExpiry(const Eigen::Vector3d& average) const;
  // Re-evaluate, on the CURRENT map, how many cells of frontier `id` are
  // visible from (pos, yaw).  -1 if the id is unknown.  Used as a pre-flight
  // check: viewpoints are sampled once at creation and go stale when walls
  // are mapped in between (a common zombie-frontier source).
  int recheckViewpointVisibility(int id, const Eigen::Vector3d& pos, double yaw);
  int minVisibNum() const { return min_visib_num_; }
  int retiredBlockedFrontiers() const { return retired_blocked_frontiers_; }
...
private:
  ...
  // Frontier-level suppression ("zombie" frontiers that persist although the
  // robot is standing at their viewpoint): a sphere around the cluster
  // average that retires any frontier inside it at the next search.
  struct BlockedFrontier {
    Eigen::Vector3d pos;
    double radius;
    double until;
  };
  vector<BlockedFrontier> blocked_frontiers_;
  int retired_blocked_frontiers_{0};
