...
private:
  ...
  // Minimum (ROS) seconds between two flood fills.  The fill scans the whole
  // box (8 M voxels for a 52 m cube) and is shared by both HGrid levels, so
  // it is recomputed at most once per interval and reused otherwise.
  double observable_unknown_interval_;
  // False when the mask was served from the throttle cache (may be stale).
  bool observable_mask_fresh_{false};
