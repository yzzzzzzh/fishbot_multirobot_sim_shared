...
namespace {
// Shared between the two HGrid levels of one exploration node: both see the
// same SDFMap, so one flood fill serves both.
struct ObservableUnknownCache {
  std::vector<uint8_t> mask;
  double stamp = -1e9;
  int observable = 0, enclosed = 0;
};
ObservableUnknownCache g_observable_cache;
}  // namespace
...
UniformGrid::UniformGrid(
  ...
  nh.param("partitioning/observable_unknown_interval", observable_unknown_interval_, 0.0);
  ...
  if (!initialized_) {
    ...
    for (auto it = grid_ids.begin(); it != grid_ids.end();) {
      if (relevant_map_.find(*it) == relevant_map_.end()) {
        if (observable_unknown_only_ && !observable_mask_fresh_ &&
            grid_data_[*it].is_prev_relevant_) {
          // The verdict came from a throttled (possibly stale) observable
          // mask: keep the cell one more cycle and force a fresh fill.
          g_observable_cache.stamp = -1e9;
          ++it;
          continue;
        }
...
void UniformGrid::updateObservableUnknownMask() {
  ...
  // Throttle: on a 200^3 box this scan+fill costs ~0.7 s and was being run
  // for every grid tour of every level (the v66 six-UAV baseline spent p50
  // 740 ms per plan in it).  Enclosed volume only appears once walls are
  // fully wrapped, so a few seconds of staleness is harmless.
  const double now = ros::Time::now().toSec();
  const ros::WallTime wall0 = ros::WallTime::now();
  auto& cache = g_observable_cache;
  if (static_cast<int>(cache.mask.size()) == n && now - cache.stamp < observable_unknown_interval_ &&
      now >= cache.stamp) {
    observable_unknown_ = cache.mask;
    observable_unknown_count_ = cache.observable;
    enclosed_unknown_count_ = cache.enclosed;
    observable_mask_fresh_ = false;
    return;
  }
  observable_mask_fresh_ = true;
  ...
  cache.mask = observable_unknown_;
  cache.stamp = now;
  cache.observable = observable_unknown_count_;
  cache.enclosed = enclosed_unknown_count_;
  ROS_WARN_THROTTLE(10.0, "RACER_METRIC observable_unknown level=%d observable=%d enclosed=%d fill_ms=%.1f",
      level_, observable_unknown_count_, enclosed_unknown_count_,
      (ros::WallTime::now() - wall0).toSec() * 1000.0);
}
