...
void FrontierFinder::searchFrontiers() {
  ...
  for (auto iter = frontiers_.begin(); iter != frontiers_.end();) {
    ...
    } else if (isFrontierBlocked(iter->average_)) {
      // Zombie frontier (see blockFrontier): move it to the dormant list
      // here, where the cost-matrix bookkeeping (removed_ids_) stays
      // consistent.  Its cells KEEP their frontier flag, so the seed scan
      // below cannot rebuild the same cluster in this very call; the flags
      // are released when the block expires (dormant loop) or when the map
      // there actually changes.
      iter->retire_until_ = frontierBlockExpiry(iter->average_);
  ...
  if (retired_now > 0) {
    retired_blocked_frontiers_ += retired_now;
    ROS_WARN("RACER_METRIC zombie_frontier_retired count=%d total=%d", retired_now,
        retired_blocked_frontiers_);
  }
  // std::cout << "After remove: " << frontiers_.size() << std::endl;
  const double retire_now = ros::Time::now().toSec();
  for (auto iter = dormant_frontiers_.begin(); iter != dormant_frontiers_.end();) {
    if (haveAnyOverlap(iter->box_min_, iter->box_max_, mins, maxs) && isFrontierChanged(*iter))
      resetFlag(iter, dormant_frontiers_);
    else if (iter->retire_until_ > 0.0 && retire_now >= iter->retire_until_ &&
             !isFrontierBlocked(iter->average_))
      resetFlag(iter, dormant_frontiers_);  // block expired: allow re-forming
    else
      ++iter;
  }
...
void FrontierFinder::computeFrontiersToVisit() {
  ...
  for (auto& tmp_ftr : tmp_frontiers_) {
    // A cluster re-formed inside a zombie block sphere stays dormant (flags
    // kept) until the block expires or its cells change.
    if (isFrontierBlocked(tmp_ftr.average_)) {
      tmp_ftr.retire_until_ = frontierBlockExpiry(tmp_ftr.average_);
...
bool FrontierFinder::isViewpointBlocked(const Eigen::Vector3d& pos) const {
  ...
  for (const auto& item : blocked_viewpoints_) {
    ...
    // 3-D test: an XY-only radius would block an infinite vertical column, i.e.
    // every stacked storey of a multi-floor building for the whole TTL.
    if ((item.first - pos).norm() < viewpoint_block_radius_) return true;
  }
  return false;
}

void FrontierFinder::blockFrontier(const Eigen::Vector3d& average, double radius, double until) {
  const double now = ros::Time::now().toSec();
  for (auto it = blocked_frontiers_.begin(); it != blocked_frontiers_.end();) {
    if (it->until <= now) it = blocked_frontiers_.erase(it);
    else ++it;
  }
  for (auto& item : blocked_frontiers_) {
    if ((item.pos - average).norm() < 0.5 * resolution_) {
      item.until = std::max(item.until, until);
      item.radius = std::max(item.radius, radius);
      return;
    }
  }
  blocked_frontiers_.push_back({ average, radius, until });
}

int FrontierFinder::recheckViewpointVisibility(int id, const Eigen::Vector3d& pos, double yaw) {
  for (auto& frontier : frontiers_) {
    if (frontier.id_ != id) continue;
    return evaluateViewpoint(pos, yaw, frontier.filtered_cells_, nullptr);
  }
  return -1;
}

double FrontierFinder::frontierBlockExpiry(const Eigen::Vector3d& average) const {
  const double now = ros::Time::now().toSec();
  double until = 0.0;
  for (const auto& item : blocked_frontiers_) {
    if (item.until <= now) continue;
    if ((item.pos - average).norm() < item.radius) until = std::max(until, item.until);
  }
  return until;
}

bool FrontierFinder::isFrontierBlocked(const Eigen::Vector3d& average) const {
  const double now = ros::Time::now().toSec();
  for (const auto& item : blocked_frontiers_) {
    if (item.until <= now) continue;
    if ((item.pos - average).norm() < item.radius) return true;
  }
  return false;
}
