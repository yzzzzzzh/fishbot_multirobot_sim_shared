...
void MapROS::visCallback(const ros::TimerEvent& e) {
  ...
  if (show_all_map_) {
    ...
    // Off by default: on a 200^3 3-D box this is a full 8 M-voxel scan per
    // second per node (a serious CPU/bandwidth cost for six UAVs).
    static double tpass2 = 0.0;
    tpass2 += (e.current_real - e.last_real).toSec();
    if (publish_unknown_ && tpass2 > 1.0) {
      publishUnknown();
      tpass2 = 0.0;
    }
  }
