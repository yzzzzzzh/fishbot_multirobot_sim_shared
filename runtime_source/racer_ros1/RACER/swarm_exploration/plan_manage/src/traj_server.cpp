...
// Running length of traj_cmd_ (incremental; the vector is capped below).
double traj_cmd_len_ = 0.0;
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  if (traj_cmd_.size() == 0) {
    ...
    traj_cmd_len_ += (pos - traj_cmd_.back()).norm();
    traj_cmd_.push_back(pos);
  ...
  // Bound the history: at 100 Hz an unbounded vector plus a full-length scan
  // per tick was O(t^2) over a long run.
  if (traj_cmd_.size() > 20000)
