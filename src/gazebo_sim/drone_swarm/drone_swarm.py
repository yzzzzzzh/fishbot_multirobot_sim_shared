...
SOLO_Z = 0.70            # lidar at 0.85 m, still under the 1.0 m wall tops
CRUISE_Z = 0.45
LAND_Z = 0.06
...
GLOBAL_TIMEOUT = T_WARM_END + 200.0
...
class Swarm(Node):
    ...
    def blocker(self, b, x, y):
        ...
        for o in BOTS:
            ...
            if d > YIELD_DIST or d > bd:
                continue
            higher = BOTS.index(o) < BOTS.index(b)
    ...
    def step(self):
        ...
        if self.phase == "TOUR":
            self.tour(now)
            if all(self.done.values()) or now > GLOBAL_TIMEOUT:
                self.get_logger().info("tours done (%.0f s) -> landing" % now)
                self.phase = "LAND"
            return
    ...
    def solo(self, b, tau):
        ...
        if tau < 3.0:                      # climb in place, clear of the others
            ...
            self.go(b, sx, sy, SOLO_Z, 0.5)
        else:                              # drop back to the parked layer
            self.go(b, sx, sy, PARK_Z, 0.4)

    def tour(self, now):
        for b in BOTS:
            x, y, z, yaw = self.gt[b]
            if self.done[b]:
                self.hold(b, CRUISE_Z)
                continue
            p = self.path[b]
            i = self.idx[b]
