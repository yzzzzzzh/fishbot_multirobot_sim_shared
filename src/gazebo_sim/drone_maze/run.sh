#!/usr/bin/env bash
# 5-drone coordinated SLAM in the TALL-WALL maze, run inside fishbot_gazebo. Requires:
#   DRONE_WORLD=drone_maze.world \
...
pkill -9 -f 'drone_maze\.py' 2>/dev/null
...
sleep 3
echo "[run] launching 5-drone maze swarm ..."
python3 -u drone_maze.py     # -u: else the report sits in stdout's block buffer
echo "[run] finished."
